...
function getRandomTetrominoType() {
    ...
    if (currentDifficulty === 'extreme') {
        const selectedTypes = getExtremeBlockTypes();
        if (selectedTypes && selectedTypes.length > 0) {
            // 从选择的方块类型中随机选择一个
            const randomIndex = Math.floor(Math.random() * selectedTypes.length);
            return selectedTypes[randomIndex];
        }
    }
...
function hardDrop() {
    ...
    let hardDropScore = Math.min(dropDistance * 2, 10);
    // ✅ 开挂模式得分 x10 倍
    if (currentDifficulty === 'extreme') {
        hardDropScore *= 10;
    }
...
async function lockPiece() {
    ...
    try {
        ...
        console.error('lockPiece 错误:', error);
    } finally {
        // 无论成功或失败，都重置锁定标志
        isLocking = false;
    }
...
async function clearLines() {
    ...
    if (linesCleared > 0) {
        ...
        // 根据消除行数和等级计算分数
        let points = SCORE_TABLE[linesCleared] * (level + 1);
        // ✅ 开挂模式得分 x10 倍
        if (currentDifficulty === 'extreme') {
            points *= 10;
        }
...
function spawnPiece() {
    ...
    nextPieceType = nextNextPieceType || getRandomTetrominoType();
