...
const CONFIG = {
    ...
    SPECIAL_FOOD: {
        ...
        INVINCIBLE: {  // 无敌豆豆
            COLOR: '#FFD700',
            RADIUS: 9,
            INVINCIBLE_DURATION: 10,  // 无敌 10 秒
            CHANCE: 0.015  // 1.5% 概率
        }
    },
...
class Game {
    ...
    updateHomeSkinDisplay() {
        ...
        // 设置渐变色背景
        display.style.background = `linear-gradient(135deg, ${currentSkin.colors[0]}22, ${currentSkin.colors[1]}44)`;
        display.style.border = `3px solid ${currentSkin.colors[0]}`;
        display.style.boxShadow = `0 0 30px ${currentSkin.colors[0]}66, inset 0 0 20px ${currentSkin.colors[1]}44`;
        ...
        for (let i = 0; i < segmentCount; i++) {
            ...
            segment.style.boxShadow = `0 0 ${size}px ${currentSkin.colors[0]}aa, 0 0 ${size*2}px ${currentSkin.colors[1]}88`;
            segment.style.border = `2px solid ${currentSkin.colors[0]}`;
        ...
        top3.forEach((record, index) => {
            ...
            item.innerHTML = `
                ...
                <span class="snake-info">
                    ...
                    <span style="font-size: 10px; opacity: 0.6; margin-left: 5px;">${difficultyNames[record.difficulty] || ''}·${new Date(record.date).toLocaleDateString()}</span>
                </span>
            `;
    ...
    addToHistory(length, time, mode, difficulty) {
        const record = {
            length: length,  // 无尽模式下是存活时间
            time: Math.floor(time),
            mode: mode,
            difficulty: difficulty || 'normal',  // 添加难度信息
    ...
    renderHistory() {
        ...
        this.history.forEach((record, index) => {
            ...
            item.innerHTML = `
                ...
                <span style="font-size: 11px; opacity: 0.6;">${difficultyNames[record.difficulty] || ''}</span>
    ...
    gameOver() {
        ...
        this.addToHistory(score, this.elapsedTime, this.mode, this.difficulty);
