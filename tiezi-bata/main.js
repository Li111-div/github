...
// ==================== Supabase 配置 ====================
const SUPABASE_URL = 'https://pwncycfjhrnxlrowinbp.supabase.co';
const SUPABASE_ANON_KEY = 'sb_publishable_ANYxis0bJjlRCsmWLFr15w_v902PU4U';
...
const LocalDB = {
    ...
    // Posts 操作
    getPosts(category = 'all', sort = 'latest', limit = 20) {
...
// 头像配置
const AVATARS = {
    avatar1: 'from-pink-400 to-rose-500',
    avatar2: 'from-blue-400 to-indigo-500',
    avatar3: 'from-green-400 to-emerald-500',
    avatar4: 'from-amber-400 to-orange-500',
    avatar5: 'from-purple-400 to-violet-500'
};
...
async function loadPosts(reset = true) {
    ...
    try {
        ...
        if (error || !posts || posts.length === 0) {
            posts = LocalDB.getPosts(currentCategory, currentSort, POSTS_PER_PAGE);
        }
        ...
        // 云端失败时直接使用本地数据
        const localPosts = LocalDB.getPosts(currentCategory, currentSort, POSTS_PER_PAGE);
    ...
    card.innerHTML = `
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
                ...
                ${post.image_url ? `<img src="${post.image_url}" class="mt-2 max-h-48 rounded-lg" alt="图片">` : ''}
...
function renderPostDetail(post, comments) {
    ...
    container.innerHTML = `
        ...
        ${post.image_url ? `<img src="${post.image_url}" class="mt-4 max-w-full rounded-lg" alt="图片">` : ''}
...
function renderComments(comments) {
    ...
    const isLove = currentCategory === 'love';
...
async function submitComment() {
    ...
    // 检查每日评论限制
    const today = getTodayDate();
    if (currentUser.last_comment_date === today && currentUser.today_comment_count >= 10) {
        document.getElementById('commentLimitWarning').classList.remove('hidden');
        showToast('今日评论已达上限（10条）', 'warning');
        return;
    }
...
async function loadMyPosts() {
    ...
    try {
        ...
        document.getElementById('profilePostCount').textContent = posts?.length || 0;
...
async function selectAvatar(avatar) {
    try {
        ...
        profileAvatar.className = `w-16 h-16 rounded-2xl flex items-center justify-center text-2xl text-white ${AVATARS[avatar]}`;
...
function handleImageUpload(input) {
    ...
    if (file.size > 1024 * 1024) {
        showToast('图片大小不能超过1MB', 'warning');
        input.value = '';
        return;
    }
    
    if (!['image/jpeg', 'image/png', 'image/jpg'].includes(file.type)) {
        showToast('只支持JPG/PNG格式', 'warning');
        input.value = '';
        return;
    }
    ...
    reader.onload = function(e) {
        ...
        fetch(e.target.result)
            .then(res => res.blob())
            .then(blob => {
                uploadedImageData = new File([blob], 'image.jpg', { type: 'image/jpeg' });
            });
    };
...
const App = {
    ...
    checkPostLimit() {
        const warning = document.getElementById('postLimitWarning');
        const text = document.getElementById('postLimitText');
        const submitBtn = document.getElementById('submitPostBtn');
    ...
    handleImageUpload(input) { handleImageUpload(input); },
