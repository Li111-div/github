...
/**
 * 选择设备类型
 * @param {string} deviceType - 设备类型 ('pc' 或 'mobile')
 */
function selectDevice(deviceType) {
    console.log('用户选择设备类型:', deviceType);
    
    // 保存设备类型到 localStorage
    localStorage.setItem('tetrisDeviceType', deviceType);
    console.log('设备类型已保存到 localStorage');
    
    // 隐藏设备选择界面
    document.getElementById('deviceSelect').style.display = 'none';
    console.log('设备选择界面已隐藏');
    ...
    if (deviceType === 'mobile') {
        ...
        console.log('控制面板 display 设置为 flex');
        initMobileControls();
        console.log('移动端控制初始化完成');
    } else {
...
function showSettings() {
    ...
    const deviceTypeElement = document.getElementById('currentDeviceType');
    if (savedDevice) {
        const deviceNames = {
            'pc': '💻 PC 端（键盘控制）',
            'mobile': '📱 移动端（触摸控制）'
        };
        deviceTypeElement.textContent = deviceNames[savedDevice] || '未知';
    } else {
        deviceTypeElement.textContent = '未选择';
    }
...
function reselectDevice() {
    console.log('用户点击重新选择设备');
    
    // 显示设备选择界面
    document.getElementById('deviceSelect').style.display = 'flex';
...
function initMobileControls() {
    ...
    // 功能键
    const btnRotateAction = document.getElementById('btnRotateAction');
    ...
    console.log('centerZone:', centerZone);
    
    // 绑定触摸事件（防止延迟）+ 点击事件（兼容性）
    if (btnLeft) {
    ...
    // 变形按钮
    if (btnRotateAction) {
        btnRotateAction.addEventListener('touchstart', (e) => {
            e.preventDefault();
            rotatePiece();
            drawBoard();
        }, { passive: false });
        
        btnRotateAction.addEventListener('click', (e) => {
            e.preventDefault();
            rotatePiece();
            drawBoard();
        });
    ...
    // 中间区域点击控制（循环切换功能）
    let clickMode = 0; // 0: 左移，1: 右移，2: 旋转，3: 下落
    if (centerZone) {
